...
export default class Settings extends Phaser.Scene {
    ...
    create() {
		...
		*/
		...
		var titleCreditsUrl = this.add.text(EPT.world.centerX, offsetTopCredits+400, 'Based on\n"Enclave Phaser Template"', fontSubtitle).setAlign('center');
		...
		var titleEffect1 = this.add.text(EPT.world.centerX, offsetTopCrew+175, 'Pillar Effect by Pipoya', fontSubtitle);
		titleEffect1.setOrigin(0.5,0).setInteractive({useHandCursor:true}).on('pointerdown',function(){this.clickEffArtist; }, this); 
	
		var titleCreditsMusic = this.add.text(EPT.world.centerX, offsetTopCrew+320, EPT.text['musicby']+' Alexandr Zhelanov' , fontSubtitle);
		titleCreditsMusic.setOrigin(0.5,0).setInteractive({useHandCursor:true}).on('pointerdown', function() { this.clickMusicArtist(); }, this);
		

		var titleSoundEffects1 = this.add.text(EPT.world.centerX, offsetTopCrew+220, 'Wrong Sfx: www.littlerobotsoundfactory.com' , fontSubtitle).setFontSize(28);
		titleSoundEffects1.setOrigin(0.5,0).setInteractive({useHandCursor:true}).on('pointerdown', function() { this.clickSound1Artist(); }, this);
	

		var titleSoundEffects = this.add.text(EPT.world.centerX, offsetTopCrew+250, 'Correct Sfx: David McKee' , fontSubtitle);
		titleSoundEffects.setOrigin(0.5,0).setInteractive({useHandCursor:true}).on('pointerdown', function() { this.clickSoundArtist(); }, this);
		
		
		var titleCreditsArt = this.add.text(EPT.world.centerX, offsetTopCrew+285, 'Art by Free Christian Illustrations' , fontSubtitle);
		titleCreditsArt.setOrigin(0.5,0).setInteractive({useHandCursor:true}).on('pointerdown', function() { this.clickArtArtist(); }, this);
		
		this.containerCredits.add([creditsBg,creditsBack,titleCredits,titleCreditsText,titleCreditsLogo,titleCreditsUrl]);
	    this.containerCredits.add([titleCrew,titleCrewAndrzej, titleCreditsMusic, titleCreditsArt, titleSoundEffects, titleSoundEffects1, titleEffect1]); //,titleCreditsMusic]);
	...
	clickSoundArtist(){
		EPT.Sfx.play('click');
		window.top.location.href= 'https://soundcloud.com/virix';
	}
	clickSound1Artist(){
		EPT.Sfx.play('click');
