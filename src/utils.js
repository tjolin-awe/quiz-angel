...
EPT.Sfx = {
    ...
    if(type == 'music' && EPT.Sfx.music['music-theme']) {
      if(EPT.Sfx.status['music']) {
        if(!EPT.Sfx.music['music-theme'].isPlaying) {
          EPT.Sfx.music['music-theme'].play({loop:true});
        }
      }
      else {
        EPT.Sfx.music['music-theme'].stop();
      }
    }
...
EPT.Lang = {
  ...
  text: {
    'en': {
      ...
      'gameplay-incorrect': 'Oh no! You got a question wrong. \n\nYou have to get every question correct to graduate to the next rank.\n\nTry again?',
      'gameplay-timeleft': 'Time: ',
      'gameplay-paused': 'PAUSED',
      'gameplay-gameover': 'Oops',
