...
export default class Story extends Phaser.Scene {
	...
	create() {
		...
		this.shine = this.add.sprite(0, 0, 'shine').setOrigin(0, 0).setAlpha(0)
										...
										self.tweens.add({
											...
											onComplete: function () {
												...
												self.tweens.add({
													...
													onComplete: function () {
														self.time.delayedCall(2000, () => {
															self.textIntro3 = self.add.text(EPT.world.centerX, self.storyIntro2.y + 150, EPT.text['story_intro3'], fontStory3).setOrigin(0.5, 0).setAlpha(0)
